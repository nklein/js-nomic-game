...
There are decision classes implemented in `src/decision.js`.
...
* `new accept_decision( augmented.id, <nullable-string> )`
* `new reject_decision( augmented.id, <nullable-string> )`
* `new defer_decision()`

To win, it would be convenient for you to emit this message:
* `new winner_decision( <name-of-winner>, <nullable-string> )`

The initial implementation **never** creates a winner decision.

### How the Initial Implementation Makes Decisions

The decision logic is implemented in the `decide()` function in `src/decide.js`.
...
The `src/players.js` contains an array of `players` and defines a function:

    (function relevant_player_login (login) ...) => the player object in the players list with that login
...
returned by the `relevant_player_count()` function in `src/players.js`.
